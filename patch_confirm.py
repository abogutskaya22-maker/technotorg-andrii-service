...
async function confirmRequest(){const data={name:s.name,phone:s.phone,machine:s.machine,brand:s.brand,model:s.model,issue:s.issue,error:s.error,changes:s.changes||'',photoData:s.photoData||'',photoName:s.photoName||''};c.innerHTML='<div style="font-weight:700;color:#666">Надсилаю заявку…</div>';try{const r=await fetch('/api/analytics',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({event:'service_request',data,occurredAt:new Date().toISOString()})});const j=await r.json().catch(()=>({}));if(!r.ok||!j.ok)throw new Error(j.error||`HTTP ${r.status}`);c.innerHTML='';b(`<b>Заявку створено та передано Андрію ✅</b><br>Він зв’яжеться з Вами за номером <b>${esc(s.phone)}</b>.${s.photoData?'<br>Фото додано до заявки.':''}`)}catch(e){c.innerHTML=`<div style="width:100%;display:grid;gap:10px"><div style="padding:12px 14px;border-radius:14px;background:#fff1f1;color:#a30f15;font-weight:700">Не вдалося передати заявку. Спробуйте ще раз.</div><button type="button" style="${requestBtnStyle(true)}" onclick="confirmRequest()">Повторити відправлення</button></div>`}}
