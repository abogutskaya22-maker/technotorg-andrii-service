...
function confirmRequest(){c.innerHTML='';trackEvent('service_request',{name:s.name,phone:s.phone,machine:s.machine,brand:s.brand,model:s.model,issue:s.issue,error:s.error,changes:s.changes||'',photoData:s.photoData||'',photoName:s.photoName||''});b(`<b>Заявку створено та передано Андрію ✅</b><br>Він зв’яжеться з Вами за номером <b>${esc(s.phone)}</b>.${s.photoData?'<br>Фото додано до заявки.':''}`)}
