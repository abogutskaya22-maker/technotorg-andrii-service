...
async function saveAnalyticsEvent(event, data, occurredAt) {
  const allowed = new Set(['page_view','consultation_started','consultation_completed','service_request','messenger_clicked','call_clicked']);
  ...
  const payload = {
    event_type: event, occurred_at: occurredAt.toISOString(),
...
export default async function handler(req, res) {
  ...
  const chatId = WORK_GROUP_CHAT_ID;
  ...
  const lines = [
    '<b>🆕 Нова заявка з сайту</b>', `🕒 ${clean(ts)}`, `👤 Ім’я: ${clean(data.name, 120)}`,
    `📱 Телефон: ${clean(data.phone, 80)}`, `🚜 Техніка: ${clean(data.machine, 120)}`,
    `🏷 Марка: ${clean(data.brand, 120)}`, `🔢 Модель: ${clean(data.model, 160)}`,
    `⚠️ Проблема: ${clean(data.issue, 700)}`, `🧾 Код помилки: ${clean(data.error, 160)}`
  ];
